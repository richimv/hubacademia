...
## 8. Anti-Repetición y Sincronización de Sesiones (Septiembre 2026)
...
2. **Exclusión Dinámica y Atómica de Casos Clínicos (Septiembre 2026)**: Al generar preguntas (`generateQuiz`), el repositorio evalúa estrictamente `isRealMock = mode === 'real'`. Para exámenes estándar (10q, 20q, modo estudio), consulta `user_question_history` para las últimas 24 horas (`seen_at > NOW() - INTERVAL '24 hours'`) y las excluye del lote candidato. Además, excluye automáticamente cualquier pregunta hermana de las casuísticas clínicas vistas (`qb.id <> ALL($N::uuid[]) AND (qb.case_id IS NULL OR qb.case_id NOT IN (SELECT case_id FROM question_bank WHERE id = ANY($N::uuid[]) AND case_id IS NOT NULL))`), erradicando casos incompletos o partidos.
3. **Fallback Resiliente**: Si el usuario agota las preguntas no vistas del banco en su especialidad o target antes de transcurrir las 24 horas, el repositorio ejecuta una consulta de reciclaje permitiendo al usuario continuar practicando sin interrupciones.
4. **Empaquetado Atómico Orgánico (`packExamQuestions`)**: Se preservan los casos clínicos íntegros tanto en configuración estándar como personalizada (`custom`), y se barajan los bloques de casos y preguntas sueltas (`shuffledUnits.flat()`) para que los casos aparezcan distribuidos en posiciones variadas e indiferentes del examen, manteniendo contiguas y en estricto orden secuencial (`case_order`) las preguntas hermanas de cada caso clínico.
5. **Exclusión en la Misma Sesión**: Al solicitar lotes adicionales (`/next-batch`) en un mismo examen, el cliente envía la lista de IDs de preguntas actualmente presentadas en la sesión (`seenIds`). El backend concatena este vector con las preguntas de las últimas 24 horas para garantizar la exclusión absoluta de preguntas ya mostradas en la sesión activa actual.
6. **Detección de Duplicados en IA (Jaccard & Normalización)**: En los flujos de generación asistida por IA (Admin y User), el backend recupera hasta 50 preguntas existentes de la base de datos para la misma área y target. Estas preguntas se inyectan en el prompt como historial prohibido. Adicionalmente, se ejecuta un algoritmo de similitud por palabras (coeficiente Jaccard > 0.65) en la fase de auditoría de calidad (`checkQuality`) que, de ser activado, obliga a la IA a regenerar el reactivo en un ciclo de refinamiento iterativo.
...
7. **Control e Interrupción de Scroll Automático y Reset al Inicio**: Para evitar conflictos de scroll cuando el usuario interactúa manualmente durante la animación automática hacia el botón Siguiente, se implementó un detector de interrupción basado en eventos de interacción del usuario (`wheel`, `touchstart`, `touchmove`, `mousedown`, `pointerdown`, `keydown`). Al dispararse cualquiera de estos eventos se ejecuta `cancelCurrentScroll()`, deteniendo de inmediato el ciclo de frames. Además, se integró la detención de cualquier scroll activo al hacer clic en el botón Siguiente y al iniciar la función `renderQuestion()`, asegurando que la transición a la siguiente pregunta restablezca instantáneamente el scroll al extremo superior de la pantalla (`window.scrollTo({ top: 0, behavior: 'instant' })`) de forma limpia y consistente.
8. **Optimización de Ancho y Spacing en Corrección de Simulacro (Móviles)**: Al ingresar a la pantalla de revisión de respuestas (`showExamReview`), se inyecta dinámicamente la clase de control `.review-mode` en el contenedor `.quiz-container`. En dispositivos móviles (ancho inferior a `640px`), esta clase reduce el padding del contenedor de `1.25rem` a `0.5rem` y amplía el ancho útil al `98%`. Asimismo, se reduce el padding interno de las tarjetas `.review-card` (de `1.25rem` a `0.85rem`) y de las explicaciones `.review-explanation` (de `1rem` a `0.75rem`), liberando hasta un 25% más de espacio de lectura horizontal para evitar la apariencia comprimida en pantallas móviles.

---

## 11. Renderizado de Tablas Markdown en Contenido HTML y Responsividad Premium (Junio 2026)
Para solucionar el problema de visualización de tablas en formato Markdown que venían encapsuladas dentro de etiquetas de párrafo de HTML (`<p>`, `</p>`) o acompañadas de etiquetas de salto de línea (`<br>`), se diseñó e implementó un motor de pre-procesamiento de tablas integrado en el renderizador unificado del sistema:
1. **Pre-procesador de Tablas Integrado (`renderMarkdownTables`)**:
   - En `markdown-renderer.js`, antes de delegar al motor DOM o a `marked.js`, se intercepta el HTML y se realiza una normalización de saltos de línea (reemplazando `<br>` por `\n` e inyectando saltos en etiquetas de bloque).
   - Se limpia cada línea descartando selectores HTML contenedores (como `<p>`, `<div>`, etc.) para verificar la validez sintáctica de la fila (`isTableRow`, `isSeparatorRow`).
   - Se parsea el bloque completo de filas y separadores markdown, y se genera un bloque HTML de tabla estructurada (`<table>` y `<tbody>`/`<thead>`) con la clase de visualización premium `.premium-table` envuelta en `.table-wrapper`.
2. **Estilo Premium y Glassmorphism (Alineado con el DESIGN_SYSTEM.md)**:
   - Se eliminaron los antiguos bordes sólidos negros y fondos blancos de las tablas en `quiz.css`, los cuales desentonaban con la estética oscura.
   - Las tablas adoptan ahora una paleta integrada de colores: fondo de superficie negro mate (`#0a0a0a`), bordes translúcidos (`1px solid rgba(255,255,255,0.05)`), y filas alternas con fondos levemente aclarados (`rgba(255,255,255,0.01)`).
   - Las celdas de cabecera usan un fondo semi-transparente azul de acento (`rgba(59, 130, 246, 0.06)`), texto en azul eléctrico brillante (`#60a5fa`) y una tipografía clara en mayúsculas con tracking expandido (`letter-spacing: 0.05em`).
3. **Responsividad Inteligente y Ancho Mínimo de Columnas**:
   - En dispositivos móviles, para evitar que las tablas de 3 o 4 columnas se compriman hasta volverse completamente ilegibles, se fijó un ancho mínimo (`min-width: 600px`) en la tabla.
