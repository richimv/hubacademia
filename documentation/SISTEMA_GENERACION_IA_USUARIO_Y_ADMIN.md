...
* **Resiliencia de Credenciales y Fallback REST de Modelos**: Para evitar cuelgues por errores de tipo `ENOENT` (archivo no encontrado) al cargar rutas locales de Windows en servidores de producción Linux (Render), se ha implementado un sanitizador global de credenciales en `server.js` (y en el constructor de `AdminAiService`) que limpia o redirige de forma segura `GOOGLE_APPLICATION_CREDENTIALS`. Adicionalmente, `_callModel` implementa un fallback a nivel de REST API para reintentar la llamada empleando `gemini-2.5-flash-lite` con la misma `GEMINI_API_KEY` si `gemini-3.1-flash-lite` no se encuentra disponible o excede la cuota de red, propagando el error original de la API de Google AI Studio en lugar de enmascararlo con fallos de inicialización de Vertex AI.
