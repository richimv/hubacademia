...
*   **Catálogos Públicos (Lectura):** Las tablas de datos estáticos y relación (`careers`, `courses`, `topics`, `resources`, `course_books`, `course_careers`, `course_topics`, `topic_resources`) permiten lectura pública (`SELECT USING (true)`) pero deniegan cualquier escritura desde la API pública PostgREST.
*   **Tablas de Autoridad y Banco de Preguntas (Privadas / Backend-Only):** `question_bank`, `quiz_sessions`, `quiz_session_questions` y `payment_events` tienen revocado todo acceso (`SELECT/INSERT/UPDATE/DELETE`) para los roles `PUBLIC`, `anon` y `authenticated` de PostgREST. Esto evita la filtración del banco de respuestas (`correct_option_index`), la alteración de sesiones de examen y la manipulación de eventos de pago. Solo el backend de Node.js interactúa con estas tablas mediante su conexión directa por pool de PostgreSQL.
