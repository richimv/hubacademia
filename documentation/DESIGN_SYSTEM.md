...
- **Cuadrícula de Puntos (Dotted Grid) y Resplandor de Acento (Glow):**
  - Se implementó un patrón de cuadrícula de puntos radiales en `.homepage-sections-container` utilizando `background-image: radial-gradient(rgba(255, 255, 255, 0.03) 1px, transparent 1px); background-size: 24px 24px;` para dar textura tecnológica de fondo.
  - Se añadieron efectos de resplandor (glow) personalizados para las tarjetas de previsualización en hover (`#docente-section:hover`, `#salud-section:hover`, `#idiomas-section:hover`, `#repaso-section:hover`, `#biblioteca-section:hover`), adaptando el `border-color` y `box-shadow` al color de acento de cada sección.
  - En la página de simuladores, el glow y los colores dinámicos se generan en hover usando la variable CSS `--accent-rgb` con valores de acento específicos para cada una de las 3 secciones por módulo (variantes de morado en Educación, variaciones de azul/celeste en Salud, y variaciones de rosa/magenta/violeta en Idiomas).
  - Se removieron los badges redundantes ("Módulo X") de la página principal e index.html, y el botón de retorno "Volver al Inicio" de la página de simuladores para lograr una cabecera más minimalista y enfocada.
  - **Optimización de Ancho Completo en PC (Simuladores):** Se configuró `.dashboard-container` en `simulators.css` para ocupar el 100% de la pantalla (`max-width: none !important`), permitiendo que los fondos radiales de acento se extiendan de borde a borde en PC de la misma manera que en la página principal. El título del hub y su subtítulo se mantuvieron centrados y alineados mediante una cabecera limpia (`.header-clean`) con `max-width: 1200px` y centrado automático.
  - **Optimización de Espaciado y Tipografía Móvil (Simuladores):** Se redujo la altura mínima excesiva en móviles de las secciones animadas (`.scroll-animate-section` con `min-height: auto !important` y padding vertical de `3rem` en `<= 991px`) para evitar grandes espacios vacíos. Se restauraron los tamaños adecuados para títulos de sección y cabecera general en móviles (`.section-main-title` de `2.1rem` en `<= 991px` y `1.75rem` en `<= 480px`, y `#sim-hub-title` de `1.9rem` y `1.65rem`) con `!important` para neutralizar el reset genérico proveniente de `dashboard.css`.
