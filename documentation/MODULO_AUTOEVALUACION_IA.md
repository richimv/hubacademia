...
3.  **Ruta B (Texto Plano >= 15,000 caracteres o NULO)**:
    -   Se activa el motor agéntico de **Recuperación Resiliente en 3 Niveles (Multi-Tier Fallback)**:
        -   **Nivel 1 (RAG Dinámico por Metadato)**: Se extraen automáticamente 5 subtemas del título del recurso. Se eligen 2 al azar y se consulta Pinecone aplicando un filtro estricto por el nombre de archivo (`filename`) para recuperar fragmentos vectorizados exactos del PDF original.
        -   **Nivel 2 (RAG Semántico Global)**: Si el Nivel 1 no devuelve resultados (por ejemplo, el recurso no está indexado en ese metadato), se realiza una búsqueda semántica amplia en toda la base vectorial combinando el título del recurso y los subtemas para rescatar contexto de soporte.
        -   **Nivel 3 (Fallback Generativo Experto)**: Si la base vectorial está caída o no hay coincidencias, el sistema actúa de forma autónoma. Gemini asume el rol de Quiz Master experto y genera un examen de alta fidelidad basado únicamente en el título del recurso y su base de conocimiento global, asegurando que el juego inicie con éxito y eliminando los callejones sin salida (dead-ends).
